...
.Env$posterpoller_tokens <- list()

# Set token to environment
set_token <- function(token, app_name = "google", in_test = FALSE) {
  .Env$posterpoller_tokens[[app_name]] <- token
...
cache_token <- function(token, app_name = "google") {
  saveRDS(token, file.path(cache_secrets_folder(), paste0(app_name, ".RDS")))
}

remove_token <- function(app_name = "google") {
  .Env$posterpoller_tokens[[app_name]] <- NULL
  googledrive::drive_deauth()
  googlesheets4::gs4_deauth()
  if (app_name == "google") options(google = NULL)
}

remove_cache <- function(app_name = "google") {
  if (app_name == "google") {
...
get_token <- function(app_name = "google", try = FALSE, silent = FALSE) {
  # If there's none in the current environment, attempt to grab a stored credential
  if (is.null(.Env$posterpoller_tokens[[app_name]])) {
    # Attempt to get stored token
    .Env$posterpoller_tokens[[app_name]] <- get_stored_token(app_name)

    # only print this message if we are successful
    if (!is.null(.Env$posterpoller_tokens[[app_name]])) message("Using user-supplied token stored using authorize(\"", app_name, "\")")
  }
  # Attempt to grab a cached credential
  if (is.null(.Env$posterpoller_tokens[[app_name]])) {
    .Env$posterpoller_tokens[[app_name]] <- get_cached_token(app_name)
  }
  # only print this message if we are successful
  if (!is.null(.Env$posterpoller_tokens[[app_name]])) {
    if (!silent) message("Using user-supplied cached tokens stored using authorize(\"", app_name, "\")")
    if (app_name == "google") {
      googledrive::drive_auth(token = .Env$posterpoller_tokens[[app_name]])
      googlesheets4::gs4_auth(token = .Env$posterpoller_tokens[[app_name]])
    }
  }
  ...
  invisible(.Env$posterpoller_tokens[[app_name]])
}

# Check if token already exists
check_for_tokens <- function(app_name = "google") {
  if (is.null(app_name)) {
...
get_stored_token <- function(app_name = "google") {
...
get_cached_token <- function(app_name = "google") {
  if (app_name == "google") {
